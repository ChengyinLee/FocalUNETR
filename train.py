...
from datasets.dataset import ProstateDataset, RandomGenerator
...
db_val = ProstateDataset(base_dir=opt['datasets']['val']['base_dir'], 
                         ...
                         transform=None)
# create data loader
loader_train = DataLoader(db_train, batch_size=opt['train']['batch_size']['train'], shuffle=True, num_workers=8)
...
for epoch in range(max_epoch):
    ...
    for batch in loader_train:
        image_batch, label_batch = batch['image'].to(device), batch['label'].to(device)
        ...
        if epoch > 10:
            ...
            if  (epoch + 1) % val_freq == 0 or (epoch >= max_epoch - 1):          
                performance, mean_hd95 = inference_online(db_val, model, 
                                                        ...
                                                        logging=logging, test_save_path=None, z_spacing=3.0)
