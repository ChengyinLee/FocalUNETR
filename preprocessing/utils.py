...
def CropForegroundFix(imImage, imLabel, fix_size=224,  context_size=[10, 30, 30]):
    ...
    dff_x = fix_size - (x_max-x_min)
    x_min = x_min - dff_x//2
    x_max = x_max + (dff_x - dff_x//2)
    ...
    if x_min <0:
        ...
        x_min = 0
    if x_max>xx:
        x_min = x_min - (x_max - xx)
        x_max = xx
    
    assert x_min>=0 and x_max <= xx and x_min < x_max
        
    
    
    dff_y = fix_size - (y_max-y_min)
    y_min = y_min - dff_y//2
    y_max = y_max + (dff_y - dff_y//2)
    ...
    if y_min <0:
        ...
        y_min = 0
    if y_max>yy:
        y_min = y_min - (y_max - yy)
        y_max = yy
    
    assert y_min>=0 and y_max <= yy and y_min < y_max
